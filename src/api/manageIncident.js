...
router.post('/generate_dailyreport', async (req, res) => {
  ...
  if(incidents.length > 0)
  {
    ...
    var text6 = 'Completion Date of Incident: ' + incidents[i].completed_at + ".";
  ...
  {
    return res.status(201).send("Daily Report cannot be generated because of 0 such records with the current date (as completed_at) on the database");
  }
...
router.post('/generate_weeklyreport', async (req, res) => {
  ...
  if(incidents.length > 0)
  {
    ...
    doc.pipe(fs.createWriteStream(dir));  
    doc.fontSize(16).text("Overall Weekly Report generated from " + olddatetime + " to " + newdatetime);
    doc.moveDown();
...
{
  return res.status(201).send("Weekly Report cannot be generated because of 0 records");
}
